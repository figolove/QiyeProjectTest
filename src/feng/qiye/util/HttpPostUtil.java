...
public class HttpPostUtil {
	...
	private void writeFileParams() throws Exception {
		...
		for (Iterator<String> it = keySet.iterator(); it.hasNext();) {
			...
			outputStream.writeBytes("--" + boundary + "\r\n");
			outputStream.writeBytes("Content-Disposition: form-data; name=\""
					+ name + "\"; filename=\"" + encode(value.getName())
					+ "\"\r\n");
	...
	public static void test(String[] args) throws Exception {
		HttpPostUtil post = new HttpPostUtil(
				"http://192.168.1.254:2627/Upload.aspx");
		post.addParameter("txtUse", "photo");
		post.addParameter("txtSessionId", "a4550ec6");
		post.addParameter("FileUpload1", new File("/mnt/sdcard/9io.jpg"));
		post.send();
	}
}
